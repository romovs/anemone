...
public class Fightview extends Widget {
	...
	public void uimsg(String msg, Object... args) {
		...
		if (msg == "new") {
			...
			try {
				Relation rel = getrel((Integer) args[0]);
				if (current != null && rel.gobid == current.gobid)
					return; // new
				makeCurrent(rel); // new
			} catch (Notfound e) {
				current = null;
			...
			/*
			 * try { Relation rel = getrel((Integer)args[0]); lsrel.remove(rel);
			 * lsrel.addFirst(rel); current = rel; curgive.state =
			 * rel.give.state; curava.avagob = rel.gobid; curava.color =
			 * rel.color(); } catch(Notfound e) { current = null; }
			 */
			return;
		} else if (msg == "atkc") {
	...
	public void attackCurrent(/* String attackName */) {// new
		 ...
		 * ){ memorized = true; }
		 * 
		 * memAttack = attackName; //System.out.println(memorized); if(current
		 * != null && !memorized){ current.ava.mousedown(Coord.z, 1); }
		 */
