...
public class ChatHWPanel extends Widget implements IHWindowParent {
	...
	static BufferedImage[] fbtni = new BufferedImage[] { Resource.loadimg("gfx/hud/fbtn"), Resource.loadimg("gfx/hud/fbtnd"),
	...
	public ChatHWPanel(Coord c, Coord sz, Widget parent) {
		...
		sub = new Button(new Coord(300, 260), sbtnw, this, Resource.loadimg("gfx/hud/slen/sau")) {
			public void click() {
				sup();
			}
		};
		sdb = new Button(new Coord(300, 280), sbtnw, this, Resource.loadimg("gfx/hud/slen/sad")) {
			public void click() {
				sdn();
			}
		};
	...
	private void updbtns() {
		...
		for (int i = 0; i < bpp; i++) {
			...
			Button b = btns.get(w);
			w.sz = sz.sub(0, btnh);
			b.change(w.title, w.visible ? Color.WHITE : SlenHud.urgcols[w.urgent]);
			b.visible = true;
			b.sz.x = bw;
