...
public class MapView extends Widget implements DTarget, Console.Directory {
	...
	public boolean mousedown(Coord c, int button) {
		...
		if (Config.minerSafety && button == 1)
			if (!checkMinesuportSafeTile(mousepos))
				return true;

		if (Config.pathDrinker)
			runFlaskScript(button); // new flask script

		modflag = ui.modflags(); // new
		if (modflag == 5)
			modflag = 0; // manually set all modflags to a new variable
	...
	private void alarm(Gob gob) {
		if (!playersSeenAlarm.contains(gob.id)) {
			...
			if (kin == null && Config.alarmunknown || kin != null && kin.group == 2 && Config.alarmred) {
	...
	private void autoaggro(Gob gob) {
		if (!playersSeenAggro.contains(gob.id)) {
			...
			if (kin == null && Config.aggrounknown || kin != null && kin.group == 2 && Config.aggrored) {
